...
import { useBookingsByEmail } from "@/src/services/useBookingsByEmail";
...
export default function MyBookingsScreen() {
    ...
    function isUpcoming(item: Booking) {
        const d = new Date(item?.date || "");
        if (Number.isNaN(d.getTime())) return false;
        return d.getTime() >= Date.now();
    }

    function isPast(item: Booking) {
        const d = new Date(item?.date || "");
        if (Number.isNaN(d.getTime())) return false;
        return d.getTime() < Date.now();
    }
    ...
    const list = useMemo(() => {
        const base = (data || []).filter((b: Booking) => {
            return (
        ...
        <View style={styles.container}>
            ...
            <FlatList
                ...
                renderItem={({ item }: { item: Booking }) => {
                    const status = item?.status || "CONFIRMÉ";
                    const date = item?.date || "15 Août • 20:00";
                    const venue = item?.venue || "Théâtre de Verdure, Agadir";
                    const title = item?.eventTitle || "La Grande Soirée Gnawa";
                    const code = item?.code || "#GNW-442";
