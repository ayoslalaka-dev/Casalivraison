...
export default function BookingFormScreen() {
  ...
  const price = (event as any)?.price ?? 150;
  const total = useMemo(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.eventCard}>
        <Image
          source={{ uri: event?.bannerUrl || "https://via.placeholder.com/120x120" }}
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          ...
          <Text style={styles.eventTitle}>{event?.title || "La Grande Soirée Gnawa"}</Text>
          <View style={styles.eventRow}>
            <Ionicons name="calendar-outline" size={14} color="#687076" />
            <Text style={styles.eventMeta}>{event?.date || "15 Juin 2024 • 20:00"}</Text>
          </View>
          <View style={styles.eventRow}>
            <Ionicons name="location-outline" size={14} color="#687076" />
            <Text style={styles.eventMeta}>{event?.location || "Théâtre de Verdure, Agadir"}</Text>
          </View>
        </View>
      </View>
